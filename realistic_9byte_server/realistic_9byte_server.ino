...
const int MySerialTX = 25;
const int MySerialDir = 10;
//if MySerialDir is high (digitalWrite(MySerialDir, HIGH);) enable RX of UART chip, if enable TX of UART chip. Based on https://www.ti.com/lit/ds/symlink/iso1500.pdf?ts=1728562758865
...
void processReceivedData(uint8_t* receivedData, size_t length) {
  ...
  digitalWrite(MySerialDir, HIGH); //enables rx of uart chip
  MySerial.write(receivedData, length);
  delayMicroseconds(400);
  digitalWrite(MySerialDir, LOW); //enables tx of uart chip
...
// Main important Callback class for data received from BLE client
